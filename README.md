...
| ----------------------------- | -------- | ----- | ----------- |
| `black(std::string)` | Sets the name that black must have | Multi-word names are supported (e.g., `John Doe`) | `[Black "..."]` |
| `black_title(std::string)` | Sets the title that black must have | e.g., `GM`, `FM`, `IM`, etc. | `[BlackTitle "..."]` |
| `day(int)` | Sets the day the game must have taken place on | `UTCDate` must be in YYYY.MM.DD format | `[UTCDate "YYYY.MM.DD"]` |
| `eco(std::string)` | Sets the ECO code the game must have | | `[ECO "..."]` |
| `opening(std::string)` | Sets the opening the game must have | Only the start of the opening string is compared to the passed string. For example, if the passed string is `French Defense`, games with the opening `French Defense` or `French Defense: Paulsen Variation` will not be filtered out | `[Opening "..."]` |
| `event(std::string)` | Sets the event the game must have taken place at | | `[Event "..."]` |
| `max_black_elo(int)` | Sets the maximum ELO black can be | The maximum value is included in the range | `[BlackElo "..."]` |
| `max_white_elo(int)` | Sets the maximum ELO white can be | The maximum value is included in the range | `[WhiteElo "..."]` |
| `min_black_elo(int)` | Sets the minimum ELO black can be | The minimum value is included in the range | `[BlackElo "..."]` |
| `min_white_elo(int)` | Sets the minimum ELO white can be | The minimum value is included in the range | `[WhiteElo "..."]` |
| `month(int)` | Sets the month the game must have taken place in | `UTCDate` must be in YYYY.MM.DD format | `[UTCDate "YYYY.MM.DD"]` |
| `round(std::string)` | Sets the round the game must have been played in | | `[Round "..."]` |
| `result(std::string)` | Sets the outcome the game must have | Standard `Result`s are `1-0`, `0-1`, `1/2-1/2`, and `*` | `[Result "..."]` |
| `site(std::string)` | Sets the website the game must have been played on | Only the start of the site string is compared to the passed string. For example, if the passed string is `https://lichess.org`, games with the site `https://lichess.org` or `https://lichess.org/xxxxxxxx` will not be filtered out | `[Site "..."]` |
| `termination(std::string)` | Sets the termination the game must have | | `[Termination "..."]` |
| `time_control(std::string)` | Sets the time control the game must have been played in | `+0` is automatically removed from `TimeControl`s and inputted time control strings. For example, `600` and `600+0` as `TimeControl`s are treated the same | `[TimeControl "..."]` |
...
| `year(int)` | Sets the year the game must have taken place in | `UTCDate` must be in YYYY.MM.DD format | `[UTCDate "YYYY.MM.DD"]` |
